...
`InMemoryJobRepository` guards writes with an `asyncio.Lock` — hence the async
signatures above. Two jobs run concurrently under the runner's semaphore, and a real
repository would do I/O here anyway, so keeping the interface async means swapping it
never touches a call site.

`ArtifactStore` stays synchronous: small local file operations, and the generator
already runs them off the event loop in a worker thread. `LocalArtifactStore` writes to
`artifacts/{job_id}/`.
