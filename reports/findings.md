...
| 11 | **A job timeout reports `internal_error`.** `JobRunner` catches `asyncio.TimeoutError` and calls `_fail` without a code, so it takes the `internal_error` default — the same code reserved for genuine bugs. | Medium | R7 says failures are named. A timeout is foreseen, not a bug. Fix is ~2 lines plus a `"timeout"` member on `FailureCode`, but that is a domain enum change (CLAUDE.md §5) and round 10's Touches do not include `app/domain/job.py` — so it is raised, not done. Found while fact-checking the README against the code. |
